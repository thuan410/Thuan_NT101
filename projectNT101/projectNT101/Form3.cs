...
namespace projectNT101
{
    public partial class Form3 : Form
    {
        ...
        private void btnEncode_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < processedInput.Length; i += 2)
                {
                    ...
                    {
                        stepResult = $"{table[rowA * 6 + (colA + 1) % 6]}{table[rowB * 6 + (colB + 1) % 6]}";
                    }
                    else if (colA == colB) // Cùng cột
                    {
                        stepResult = $"{table[((rowA + 1) % 6) * 6 + colA]}{table[((rowB + 1) % 6) * 6 + colB]}";
                    }
                    else // Khác hàng, khác cột
                    {
                        stepResult = $"{table[rowA * 6 + colB]}{table[rowB * 6 + colA]}";
                    }
        ...
        private void btnDecode_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < processedInput.Length; i += 2)
                {
                    ...
                    if (rowA == rowB) // Cùng hàng
                    {
                        colA = (colA + 5) % 6; // Di chuyển ngược lại
                        colB = (colB + 5) % 6;
                        stepResult = $"{table[rowA * 5 + colA]}{table[rowB * 5 + colB]}";
                    }
                    else if (colA == colB) // Cùng cột
                    {
                        rowA = (rowA + 5) % 6; // Di chuyển ngược lại
                        rowB = (rowB + 5) % 6;
                        stepResult = $"{table[rowA * 6 + colA]}{table[rowB * 6 + colB]}";
                    }
                    else // Khác hàng, khác cột
                    {
                        stepResult = $"{table[rowA * 6 + colB]}{table[rowB * 6 + colA]}";
        ...
        private static string[,] GenerateCipherTable(string keyword)
        {
            ...
            if (unique.Length < 36)
                throw new Exception("Bảng ký tự không đủ để tạo bảng Playfair 6x6.");
        ...
        private static string ProcessMessage(string message, string[,] table, bool isEncoding)
        {
            ...
            for (int i = 0; i < message.Length; i += 2)
            {
                ...
                {
                    result += table[pos1.X, (pos1.Y + (isEncoding ? 1 : 5)) % 6];
                    result += table[pos2.X, (pos2.Y + (isEncoding ? 1 : 5)) % 6];
                }
                else if (pos1.Y == pos2.Y) // Cùng cột
                {
                    result += table[(pos1.X + (isEncoding ? 1 : 5)) % 6, pos1.Y];
                    result += table[(pos2.X + (isEncoding ? 1 : 5)) % 6, pos2.Y];
                }
                else // Hình chữ nhật
                {
        ...
        private void GenerateCipherTable1(string keyword)
        {
            ...
            string keyString = new string(keyword.Distinct().ToArray()); // Loại trùng
            string remaining = new string(alphabet.Except(keyString).ToArray()); // Lấy ký tự còn lại
            keyString += remaining;
            ...
            {
                MessageBox.Show("Không đủ ký tự để tạo bảng 6x6!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private (int, int) FindPosition(char c, string table)
        {
            ...
            return (index / 6, index % 6); // Trả về hàng và cột
        }
        ...
        private string PrepareInput(string input)
        {
            ...
            input = new string(input.Where(c => char.IsLetterOrDigit(c)).ToArray()); // Loại bỏ ký tự không phải chữ hoặc số
